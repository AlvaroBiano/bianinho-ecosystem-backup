...
- AionUI Teams architecture: `references/aionui-teams-architecture.md`
...
## AionUI Teams — Hermes Team Leader Integration

### Contexto
O AionUI expõe um sistema de Teams onde um **Team Leader** (agente) controla um team de agentes. O TEN Team foi criado com `agentType=hermes` como líder. O AionUI tenta fazer spawn do Hermes como leader usando `--experimental-acp` como argumento CLI.

### Problema: Protocolo Incompatível

O AionUI Teams espera que o agente leader exponha um **servidor MCP stdio** que se liga a um TCP server interno do AionUI via `AION_MCP_PORT`. O fluxo:

1. AionUI faz spawn do agente leader com `--experimental-acp` como env var
2. O agente deve iniciar o `team-guide-mcp-stdio.js` como stdio MCP server
3. Esse MCP server liga-se ao TCP server interno do AionUI em `AION_MCP_PORT`
4. Expõe ferramentas: `aion_create_team`, `aion_list_models`

**O Hermes usa ACP (Agent Client Protocol), não MCP stdio.** O `acp_adapter.entry` é um servidor ACP, não um servidor MCP stdio. São protocolos incompatíveis.

### Fix Parcial — Adicionar Suporte ao `--experimental-acp`

O Hermes não reconhecia o flag `--experimental-acp`. Foram necessárias duas alterações:

**1. Adicionar o argumento em `hermes_cli/_parser.py`:**
```python
parser.add_argument(
    "--experimental-acp",
    action="store_true",
    default=False,
    help="Start Hermes in ACP mode for AionUI Teams integration",
)
```

**2. Adicionar o handler em `hermes_cli/main.py`:**
```python
if getattr(args, "experimental_acp", False):
...
Verificar:
```bash
~/.hermes/venv/bin/hermes --experimental-acp --help
# Deve aparecer --experimental-acp na lista de argumentos
```
...
| Conceito | Descrição |
|----------|-----------|
| `team-guide-mcp-stdio.js` | Script MCP stdio que comunica com o AionUI via TCP |
| `AION_MCP_PORT` | Porta TCP do servidor interno do AionUI |
| `AION_MCP_TOKEN` | Token de autenticação para o TCP server |
| `KNOWN_TEAM_CAPABLE_BACKENDS` | Set em `app.asar` com backends suportados — `"hermes"` já adicionado |
| `presetMcpServers` | Configuração de servers MCP injetados pelo AionUI no agente |

### Ficheiros Modificados

- `~/.hermes/hermes-agent/hermes_cli/_parser.py` — adicionado `--experimental-acp`
- `~/.hermes/hermes-agent/hermes_cli/main.py` — adicionado handler para `--experimental-acp`
- `/Applications/AionUI.app/Contents/Resources/app.asar` — `"hermes"` adicionado a `KNOWN_TEAM_CAPABLE_BACKENDS` (modificação anterior)

---

## AionUI Hermes Shield — PROTEÇÃO CRÍTICA

### O Problema: Hermes Desaparece do Teams UI

O Hermes pode desaparecer da lista de Team Leaders disponíveis no AionUI. Isto acontece quando:
1. O AionUI é atualizado e substitui o `app.asar`
2. A modificação a `KNOWN_TEAM_CAPABLE_BACKENDS` é perdida

### ⚠️ CRÍTICO: `app.asar.unpacked` vs `app.asar`

**Modificações a `app.asar.unpacked` NÃO afetam o app em execução!**

O AionUI usa o ficheiro `app.asar` diretamente. O diretório `app.asar.unpacked` é apenas para desenvolvimento/debug. Para aplicar modificações永久:

1. Extrair: `npx asar extract app.asar app.asar.unpacked`
2. Modificar ficheiros em `app.asar.unpacked/out/main/index.js`
3. Re-embrulhar: `npx asar pack app.asar.unpacked app.asar`
4. **Reiniciar o AionUI**

### Script de Blindagem

```bash
# Verificar se Hermes está protegido
~/.hermes/scripts/aionui_hermes_shield.sh --verify

# Aplicar proteção
~/.hermes/scripts/aionui_hermes_shield.sh --apply

# Modo monitor (corre continuamente)
~/.hermes/scripts/aionui_hermes_shield.sh --watch
```

O script:
- Verifica se "hermes" está em `KNOWN_TEAM_CAPABLE_BACKENDS`
- Se desaparecer, re-aplica automaticamente
- Faz backup do `app.asar` antes de modificar
- Corre a cada 60s em modo watch

### Verificação Manual

```bash
# Verificar linha KNOWN_TEAM_CAPABLE_BACKENDS
grep "KNOWN_TEAM_CAPABLE_BACKENDS" "/Applications/AionUI.app/Contents/Resources/app.asar.unpacked/out/main/index.js"

# Deve mostrar: new Set(["gemini", "claude", "codex", "aionrs", "hermes"])
```

### Cron Job de Monitorização

O shield corre em modo watch via cron (reinicia a cada hora para evitar drift):

```bash
# O script `aionui_hermes_shield.sh --watch` é o daemon de proteção
# Deve ser posto a correr via launchd ou restart automático
```

---

## Team Leader Merged Sessions — Solução Robusta

### O Problema

O Team Leader e o main Hermes são **processos SEPARADOS**:
- Team Leader: spawned pelo AionUI via `team-mcp-stdio.js`
- Main Hermes: processo separado conectado via ACP
- Cada um tem sessão ACP diferente
- **Não há canal nativo de comunicação entre eles**

### Solução: Polling da Base de Dados

A solução mais robusta é usar a base de dados SQLite do AionUI como intermediário:

```
Team Leader → AionUI DB (mailbox/messages) → Bridge Script → Main Hermes → Álvaro
```

### Scripts Criados

1. **`team_leader_session_bridge.py`** — polling da BD
   ```bash
   # Ver estado
   python3 ~/.hermes/scripts/team_leader_session_bridge.py --status

   # Ver novas mensagens
   python3 ~/.hermes/scripts/team_leader_session_bridge.py --poll

   # Resumo completo
   python3 ~/.hermes/scripts/team_leader_session_bridge.py --summary

   # Modo monitor
   python3 ~/.hermes/scripts/team_leader_session_bridge.py --monitor
   ```

2. **`aionui_hermes_shield.sh`** — blindagem do Hermes

### Cron Job

```bash
# Adicionado: corre a cada 2 minutos
*/2 * * * * cd ${HOME} && ${HOME}/.hermes/venv/bin/python3 ${HOME}/.hermes/scripts/team_leader_session_bridge.py --cron
```

### Tabelas da BD Relevantes

| Tabela | Uso |
|--------|-----|
| `messages` | Mensagens por conversation_id |
| `mailbox` | Mensagens inter-agentes (to_agent_id, from_agent_id) |
| `conversations` | Meta-info das conversas |

### Conversation ID do Team Leader

```
TEN_TEAM_LEADER_CONV = "d124e72a"
slot do Team Leader = "slot-1df887d8"
```
...
|--------|-------------|-----------|
| `aionui_hermes_shield.sh` | `~/.hermes/scripts/` | Blindagem Hermes no Teams |
| `team_leader_session_bridge.py` | `~/.hermes/scripts/` | Polling BD para merged sessions |

Ver scripts completos em `references/aionui_hermes_shield.sh`.
