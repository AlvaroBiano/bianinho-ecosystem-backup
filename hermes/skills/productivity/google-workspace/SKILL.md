...
- ❌ Scopes individuais inválidos causam `Error 400: invalid_scope`
...
1. Generate auth URL and send to Álvaro (NEVER auto-open browser)
2. Ask Álvaro to paste URL into HIS browser
3. Ask Álvaro to paste back the redirected URL with `code=`
...
1. **Client secret MUST be Desktop app type** — NOT Web application
2. **JSON field must be `installed`** with redirect_uris http localhost
3. **Code expires quickly** — generate URL and exchange for token IMMEDIATELY
4. **Scopes must be requested together** in first authorization
5. **Create document**: `body = {"title": "..."}` — DO NOT use `{"properties": {"title": ...}}` (error 400: "Unknown name 'properties'")
6. **insertText**: up to 1MB per request — 60k chars inserted at once worked
7. **Rate limit 429**: insert in ~400-char chunks with 1.5s delay between each

### Flow
1. Create OAuth client (Desktop app type) in Google Cloud Console
2. Add account as test user: OAuth consent screen
3. Generate auth URL with PKCE (code_verifier + code_challenge)
4. Open URL in browser logged into correct account
5. Copy code from redirect URL (localhost/?code=...)
6. Exchange code for token immediately

### Google Docs API
- Create document via Drive API (`mimeType: application/vnd.google-apps.document`)
- Insert text via `batchUpdate` with `insertText`
- Scope: `documents` for write, `drive.file` for file creation
...
token = t.get('token', t.get('access_token', ''))  # try both
```
...
1. Download file: `GET /drive/v3/files/{id}?alt=media`
2. Create new file in destination folder: `POST /drive/v3/files` with `parents: [destination_id]`
3. Upload content: `PATCH /drive/v3/files/{new_id}?uploadType=multipart` with multipart body
4. Delete original: `DELETE /drive/v3/files/{original_id}`
