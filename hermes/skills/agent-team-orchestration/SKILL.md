...
### Not verifying outputs after subagent completion
Subagents complete and report success, but may create files with errors (wrong imports, non-existent icons, broken syntax) or in unexpected locations. **Always run `git status --short` after every subagent batch** before assuming work is done. If build fails, fix the errors (common: icon names not verified) and rebuild before reporting completion.

### Not including icon verification in subagent prompts
When delegating UI work that uses `@icon-park/react`, provide the verified icon list in the context. Subagents will use icons like `Edit2`, `Trash`, `Clock` without checking if they exist — causing build failures after the agent reports "done." Include the verified icons list AND the list of non-existent icons in the delegation prompt.

### Max 3 concurrent children — plan accordingly
`delegate_task` supports max 3 parallel subagents. With 5 tasks, 2 queue indefinitely. Split into batches of 3 and run sequentially or in sequence + next batch.
...
- **One-off task delegation** — Use `delegate_task()` directly. This skill is for sustained workflows with multiple handoffs.
