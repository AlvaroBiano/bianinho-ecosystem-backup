...
metadata:
  hermes:
    requires:
      ...
      config: []
    user-invocable: true
  homepage: Adapted for Hermes from github.com/yoder-bawt/learning-loop
---
...
```bash
python3 ~/.hermes/scripts/learning_loop/init.py /path/to/workspace
```
...
Add to your agent's boot instructions (or use Hermes boot system):
...
**Daily (e.g. 4am):**
```bash
python3 ~/.hermes/scripts/learning_loop/extract.py /path/to/workspace
```

**Weekly (e.g. Sunday 10pm):**
```bash
python3 ~/.hermes/scripts/learning_loop/detect_patterns.py /path/to/workspace
python3 ~/.hermes/scripts/learning_loop/confidence_decay.py /path/to/workspace
python3 ~/.hermes/scripts/learning_loop/promote_rules.py /path/to/workspace
python3 ~/.hermes/scripts/learning_loop/self_audit.py /path/to/workspace
python3 ~/.hermes/scripts/learning_loop/update_metrics.py /path/to/workspace
```
...
- ✓ Use wal-capture.py for critical details that must survive compaction
- ✓ Keep events.jsonl append-only; never delete or edit historical events
- ✓ Run confidence-decay.py weekly to update rule confidence scores
- ✓ Export rules for cross-agent sharing using export_rules.py
...
- ✗ Delete events to "clean up" - use archive-events.py for old data instead
- ✗ Assume lessons apply universally without considering context
- ✗ Manually edit rules.json - let promote_rules.py handle promotion
...
The daily extraction runs `extract.py`, which scans your session logs and flags patterns. You extract a structured lesson:
...
The weekly cron runs `promote_rules.py`. It finds L-001 with 3+ applications and confidence >= 0.9, and auto-promotes it:
...
After 30 days without validation, `confidence_decay.py` runs and applies exponential decay:
...
`update_metrics.py` tracks the trend:
- Week 1: 3 mistakes, 0 rules
- Week 2: 1 mistake, 5 rules, 2 pre-action saves
- Week 3: 0 mistakes, 8 rules, 4 pre-action saves

`self_audit.py` scores your loop health: 100% means everything is wired correctly.

**That's the full cycle.** Raw experience becomes structured knowledge becomes enforced behavior. Compounding intelligence.

## Scripts Reference

All scripts are Python and live in `~/.hermes/scripts/learning_loop/`:

| Script | Purpose | Schedule |
|--------|---------|----------|
| `init.py` | Initialize directory structure | Once |
| `extract.py` | Scan logs for uncaptured events | Daily |
| `detect_patterns.py` | Tag clusters, regressions, anomalies | Weekly |
| `confidence_decay.py` | Apply Ebbinghaus decay to confidence scores | Weekly |
| `export_rules.py` | Export rules for cross-agent sharing | Manual |
| `import_rules.py` | Import rules with conflict detection | Manual |
| `promote_rules.py` | Promote lessons to rules | Daily/Weekly |
| `self_audit.py` | Health score (23 checks, A-D) | Weekly |
| `update_metrics.py` | Weekly metrics snapshot | Daily/Weekly |
| `feedback_detector.py` | Detect human signals | Per-message |
| `track_violations.py` | Link mistakes to rules | Daily |
| `track_applications.py` | Track lesson applications | Daily |
| `rule_check.py` | Dynamic rule lookup | On-demand |
| `archive_events.py` | Roll off old events | Monthly |
| `wal_capture.py` | Write-Ahead Log capture | Per-message |
| `inject_rules.py` | Inject rules into agent context | On-demand |
...
cursor = conn.cursor()
# Query recent sessions for learning extraction
cursor.execute("SELECT content FROM sessions WHERE timestamp > datetime('now', '-7 days')")
```

### Knowledge Base Integration

Store learned patterns in `~/KnowledgeBase/` for semantic retrieval:

```bash
# Query the Knowledge Base for related learnings
python3 ~/KnowledgeBase/query.py --semantic "macOS find command issues"
```

### Hermes Boot System

Integrate with Hermes skill_manage for automatic rule loading:

```bash
# Register learning loop as a boot skill
hermes skills enable learning-loop
```
...
```bash
# Export all rules
python3 ~/.hermes/scripts/learning_loop/export_rules.py /path/to/workspace --output my-rules.json

# Export only shell rules
python3 ~/.hermes/scripts/learning_loop/export_rules.py /path/to/workspace --category shell --output shell-rules.json
```

**Import rules:**
```bash
# Preview import
python3 ~/.hermes/scripts/learning_loop/import_rules.py /path/to/workspace other-agent-rules.json --dry-run

# Import with custom trust threshold
python3 ~/.hermes/scripts/learning_loop/import_rules.py /path/to/workspace other-agent-rules.json --trust 0.8

# Import all rules above default threshold (0.5)
python3 ~/.hermes/scripts/learning_loop/import_rules.py /path/to/workspace other-agent-rules.json
```
       ...
       for line in f:
           ...
           if line:
               try:
                   ...
                   pass
   ```
4. If still broken, archive and start fresh with `init.py`
...
2. Check boot instructions include: `Read memory/learning/rules.json`
...
2. Run `init.py` to backfill missing fields
3. Check that rules are older than 1 day (no decay on same-day rules)
4. Verify `confidence_decay.py` is in your weekly cron
...
- An agent that reads files at boot

No external APIs. No dependencies beyond Python. Runs anywhere.
