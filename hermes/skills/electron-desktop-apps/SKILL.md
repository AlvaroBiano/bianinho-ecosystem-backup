...
7. **macOS Electron app patching needs auto-restore via launchd** — if you patch an Electron app's `.app` contents (e.g. add Hermes to a hardcoded agent Set in the renderer bundle), the patch is lost on every update. Use a LaunchAgent with `StartInterval: 300` + a restore script that detects and re-applies the patch. See `aionui` skill §23 for the full pattern.
