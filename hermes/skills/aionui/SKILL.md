...
### Modo 2: DMG no Mac — Local (100% offline, 02/05/2026)

O AionUI corre como app Electron nativa no Mac. Liga-se ao BianinhoBridge HTTP em `localhost:18743` — sem dependência do servidor.

**Arquitectura:**
```
MacBook local
  ├── Hermes Agent ( ~/.hermes/venv/bin/hermes )
  ├── RAG Server (localhost:3101) → ~/KnowledgeBase/knowledge_db/ (1.9GB)
  ├── BianinhoBridge HTTP (localhost:18743) → ~/.hermes/scripts/bianinho_bridge_server.py
  └── AionUI app (/Applications/AionUI.app)
```

**Setup (02/05/2026):**
1. BianinhoBridge: `~/.hermes/scripts/bianinho_bridge_server.py` com `BIND_HOST = "127.0.0.1"`
2. RAG Server: `~/KnowledgeBase/rag_server.py`
3. AionUI: build local com IP hardcoded `100.79.189.95` → `127.0.0.1` patcheado
4. launchd: `~/Library/LaunchAgents/com.bianinho.bridge.plist` + `com.bianinho.rag.plist`

**⚠️ launchd Restart Loop:** `KeepAlive=true` + loop interno do bridge causa reinícios infinitos. NÃO usar KeepAlive. Em vez disso, o bridge já tem `Restart=on-failure` internamente.

**⚠️ curl hangs no macOS:** `curl` do terminal macOS não funciona para localhost:18743 (mesmo com `--connect-timeout`). Usar Python:
```python
import urllib.request
resp = urllib.request.urlopen('http://127.0.0.1:18743/ping', timeout=5)
```

### Modo 3: DMG no Mac → Servidor (Tailscale)

O AionUI no Mac liga-se ao BianinhoBridge HTTP no servidor via Tailscale (`100.79.189.95:18743`).
...
## Teams MCP — EACCES Permission Denied (CRÍTICO — 03/05/2026)

**Sintoma:** Hermes aparece como Team Leader mas `team_spawn_agent`, `team_members`, `team_task_list` não estão disponíveis. Erro no log:
...
**Causa:** O ficheiro `~/.aionui-config/aionui-config.txt` tem permissões só de leitura (`-r--r--r--`). Quando o Hermes inicia como Team Leader, o AionUI tenta cachear o resultado de inicialização neste ficheiro — como não consegue escrever, o `hermes_team_mcp_stdio.py` nunca chega a iniciar e as tools `team_*` ficam indisponíveis.

**Fix (sempre executar primeiro quando Teams MCP falha):**
```bash
...
**Verificação:**
```bash
ls -la ~/.aionui-config/aionui-config.txt
# Deve mostrar -rw-r--r-- (não -r--r--r--)
```

**Fluxo de debugging Teams MCP (sempre nesta ordem):**
1. `chmod u+w ~/.aionui-config/aionui-config.txt` —权限
2. `grep -i "TeamGuide\|team.*mcp\|EACCES" ~/Library/Logs/AionUi/$(date +%Y-%m-%d).log | tail -20` — logs
3. `ps aux | grep -E "team-mcp|hermes_team"` — processos
4. Se `hermes_team_mcp_stdio.py` não está a correr, o AionUI não está a injetar o Teams MCP no Hermes

**Arquitectura Teams MCP (03/05/2026):**
- `team-mcp-stdio.js` → para aionrs, gemini, claude (Node.js)
- `hermes_team_mcp_stdio.py` → para Hermes (Python, TCP 4-byte length prefix + JSON)
- Ambos comunicam com o `TeamGuideMcpServer` interno do AionUI via TCP (porta dinâmica)
- O AionUI passa `TEAM_MCP_PORT` + `TEAM_MCP_TOKEN` como variáveis de ambiente

## Preferências de Comunicação do Álvaro

- **Respostas curtas e directas** — sem explicações longas, sem "deixa-me ver", sem rodeios
- **Quando algo não aparece na UI, PERGUNTAR primeiro** — não vasculhar código antes de confirmar o que o utilizador vê. Exemplo: "Hermes não aparece no Teams" → primeiro perguntar "o que aparece no dropdown? Algum agente?" antes de extrair ASARs e greppar código.
...
### MacBook Local (100% offline)
| Componente | Estado | Porta |
|-----------|--------|-------|
| Hermes Agent | ✅ Funcional | — |
| BianinhoBridge HTTP | ✅ Funcional | localhost:18743 |
| RAG HTTP Server | ✅ Funcional | localhost:3101 |
| AionUI DMG | ✅ Instalado | /Applications/AionUI.app |
| Knowledge Base | ✅ 1.9GB synced | ~/KnowledgeBase/ |

### Servidor Linux
| Componente | Estado | Porta |
|-----------|--------|-------|
| BianinhoBridge HTTP | ✅ Funcional | 100.79.189.95:18743 |
| AionUI Xvfb + Web Viewer | ✅ Funcional | localhost:8765 |
| cloudflared tunnel | ✅ activo | aionui.masterclasslife.com.br |

**Nota:** Ambos os ambientes coexistem. MacBook é 100% local; servidor é para acesso remoto via web viewer.
...
### Regra de Ouro: EXECUTAR primeiro, confirmar depois

**Quando o Álvaro diz "configure X", "faça Y", "instale Z" — executar imediatamente, não perguntar se deve fazer ou qual modo prefere.** Perguntas de clarificação só se ele pedir para escolher entre opções claramente diferentes. Se ele diz "configura como fallback", isso é uma instrução, não uma pergunta.

**Excepção:** Só perguntar se as opções tiverem consequências muito diferentes (ex: "isto apaga dados?" ou "qual é o IP do servidor?").

**Exemplo de violação (02/05/2026):** O Álvaro disse "configura o Anything LLM como fallback" e eu respondi com uma pergunta "qual preferes?" em vez de simplesmente configurar. Corrigido — executou-se e confirmou-se depois.
...
### 14. launchd plist + KeepAlive causa restart loop

**Sintoma:** O BianinhoBridge ou RAG Server são iniciados múltiplas vezes por segundo via launchd, com logs "Iniciando BianinhoBridge HTTP Server" repetidos.

**Causa:** `KeepAlive: true` no plist + loop interno do Python server (que também tenta re-iciar em caso de falha). O launchd inicia novo processo, o processo anterior ainda está a correr, ambos competem pela porta.

**Solução:** NÃO usar `KeepAlive: true` com launchd para serviços que já têm restart logic interno. O BianinhoBridge já tem `Restart=on-failure` no código. Usar apenas `RunAtLoad: true` e deixar o serviço gerir os seus próprios restarts.

```xml
<!-- ERRADO — causa restart loop -->
<key>KeepAlive</key>
<true/>
...
**Nota:** O `RestartSec=10` em systemd (servidor) é diferente — lá o processo não tem restart logic interno, por isso o systemd faz o restart. No Mac com launchd, se o processo já tem restart intern, não adicionar KeepAlive.

### 15. GitHub clone lento/timeout → usar rsync do servidor

**Sintoma:** `git clone --depth=1` do GitHub demora >60s e dá timeout no terminal.

**Solução:** Se o código já existir no servidor Linux, usar rsync direto:
```bash
rsync -avz --exclude='node_modules' --exclude='.git' --exclude='out' \
  alvarobiano@100.79.189.95:~/repos/aionui-custom/ /tmp/aionui-mac/
```

### 16. bun em novo shell — path não persistido

**Sintoma:** `bun: command not found` após instalar.

**Causa:** `curl -fsSL https://bun.sh/install | bash` instala em `~/.bun/bin/` mas o PATH não é persistido entre sessões shell.

**Solução:** Sempre definir path manualmente:
```bash
...
### 19. Teams — Hermes não aparece como Team Leader (03/05/2026)

**REGRA DE OURO: PERGUNTAR PRIMEIRO, INVESTIGAR DEPOIS**

Quando algo não aparece na UI, SEMPRE perguntar ao utilizador o que ele vê ANTES de vasculhar código ou extrair ASARs.

**O problema tinha DUAS causas raiz em localizações diferentes (03/05/2026):**

1. **Main process** (`out/main/index.js`): `KNOWN_TEAM_CAPABLE_BACKENDS` já tinha 'hermes' ✓
2. **Renderer bundle** (`out/renderer/assets/index-*.js`): `aAt = new Set([...])` NÃO tinha 'hermes' ✗

O Teams Leader dropdown usa o Set do **renderer**, não do main process. Por isso Hermes aparecia em New Chat (que usa main process data) mas não em Teams Leader.

**Sintoma:** Hermes detectado (3 agents), Hermes aparece em New Chat, mas Hermes NÃO aparece no Teams Leader dropdown.

**Fix — duas localizações precisam de patch:**

```bash
# 1. Extrair app.asar
npx asar extract "/Applications/AionUi.app/Contents/Resources/app.asar" /tmp/aionui-patch

# 2a. Patch main process (se necessário)
sed -i 's/gemini", "claude", "codex", "aionrs"/gemini", "claude", "codex", "aionrs", "hermes"/g' \
  /tmp/aionui-patch/out/main/index.js

# 2b. Patch renderer bundle (O SET QUE IMPORTA)
# Primeiro identificar o bundle (o hash muda entre versões)
RENDERER_BUNDLE=$(ls /tmp/aionui-patch/out/renderer/assets/index-*.js | head -1)
sed -i 's/aAt=new Set(\["gemini","claude","codex","aionrs"\])/aAt=new Set(["gemini","claude","codex","aionrs","hermes"])/g' \
  "$RENDERER_BUNDLE"
# 3. Repack
npx asar pack /tmp/aionui-patch "/Applications/AionUi.app/Contents/Resources/app.asar"
# 4. Re-assinar (OBRIGATÓRIO)
codesign -f -s - /Applications/AionUi.app
```

**Nota:** O `aAt = new Set(...)` no renderer é **hardcoded no bundle compilado** — não existe correspondência directa no source TypeScript. O `KNOWN_TEAM_CAPABLE_BACKENDS` em `teamTypes.ts` é para o main process, não para o renderer. O patch ao bundle é sempre necessário mesmo que o source TypeScript já tenha 'hermes'.

**ALTERNATIVA: Patch à função isTeamCapableBackend (03/05/2026 noite)**

Se o patch ao Set não funcionar, modificar directamente a função para forçar 'hermes' a passar:

```bash
# Extrair
npx asar extract "/Applications/AionUi.app/Contents/Resources/app.asar" /tmp/aion_hermes_fix

# Patch: adicionar backend === "hermes" como caso especial
sed -i '' 's/if (KNOWN_TEAM_CAPABLE_BACKENDS.has(backend)) return true;/if (KNOWN_TEAM_CAPABLE_BACKENDS.has(backend) || backend === "hermes") return true;/g' \
  /tmp/aion_hermes_fix/out/main/index.js

# Repack
cd /tmp/aion_hermes_fix
npx asar pack out "/Applications/AionUi.app/Contents/Resources/app.asar"

# Re-assinar
codesign -f -s - /Applications/AionUi.app
```

**Blindagem permanente:** ver secção 23 (Auto-restore do patch após actualizações).

**Blindagem permanente:** ver secção 23 (Auto-restore do patch após actualizações).

**Sessão:** 03/05/2026
### 20. DMG app.asar patching — NUNCA rebuild, usar asar extract/pack

**Problema:** Para adicionar features ao AionUI DMG, a tentação é fazer `bunx electron-vite build` e re-pack. ISSO QUEBRA TUDO.

**O que acontece:**
1. Rebuild cria `app.asar.unpacked/` com módulos nativos
2. `better-sqlite3` rebuild falha com `Node API incompatibility` (Node v136 vs Electron v136)
3. Mesmo que rebuild passe, o `app.asar.unpacked/` quebra a code signature do macOS
4. App fica sem código válido → não abre

**Solução CORRECTA — patch directo no ASAR:**

```bash
# 1. Extrair
npx asar extract "/Applications/AionUi.app/Contents/Resources/app.asar" /tmp/app-patched

# 2. Editar o ficheiro necessário (ex: adicionar 'hermes' a KNOWN_TEAM_CAPABLE_BACKENDS)
sed -i 's/gemini", "claude", "codex", "aionrs"/gemini", "claude", "codex", "aionrs", "hermes"/' \
  /tmp/app-patched/out/main/index.js

# 3. Repack
npx asar pack /tmp/app-patched "/Applications/AionUi.app/Contents/Resources/app.asar"

# 4. Re-assinar (OBRIGATÓRIO após qualquer modificação)
codesign -f -s - /Applications/AionUi.app
...
**ATENÇÃO:** `app.asar.unpacked/` NUNCA deve existir no packaged app — cria-se durante o rebuild e quebra a assinatura. Se existir, apagar:
```bash
rm -rf "/Applications/AionUi.app/Contents/Resources/app.asar.unpacked"
```

**Backup do ASAR original — SEMPRE fazer antes de patchar:**
```bash
cp "/Applications/AionUi.app/Contents/Resources/app.asar" \
   "/Applications/AionUi.app/Contents/Resources/app.asar.backup_before_$(date +%Y%m%d)"
```

**Sessão:** 03/05/2026

### 21. Code signing broken after app modification

**Sintoma:** App não abre após modificação — desaparece do Dock imediatamente.

**Causa:** Qualquer modificação no `.app` contents quebra a code signature do macOS (mesmo trocar um ficheiro dentro do .app).
...
**Verificação:**
```bash
codesign -vv /Applications/AionUi.app
# "valid on disk" + "satisfies its Designated Requirement" = OK
# "a sealed resource is missing or invalid" = BROKEN
```

**Sessão:** 03/05/2026

### 22. Hermes symlink — PATH do Electron não inclui ~/.hermes/venv/bin

**Problema:** O Electron (AionUI) detecta agentes via `command -v`. O `~/.hermes/venv/bin` não está no PATH do Electron.

**Solução — symlink em ~/.local/bin (que já está no PATH do Electron):**
```bash
mkdir -p ~/.local/bin
ln -sf ~/.hermes/venv/bin/hermes ~/.local/bin/hermes
# Verificar: command -v hermes (do PATH do Electron)
zsh -c 'source /etc/zshrc; command -v hermes'
```

**Sessão:** 03/05/2026

**Sintoma:** App não abre após instalação via DMG — "não é possível abrir porque berasal de um developer não identificado".

**Solução:**
```bash
sudo xattr -rd com.apple.quarantine /Applications/AionUI.app
# Se sudo pedir password (ambiente CI), usar:
xattr -rd com.apple.quarantine /Applications/AionUI.app
# (pode funcionar sem sudo se SIP permitir)
```
...
### 19. extractBianinhoResources() — extracção de recursos no primeiro lançamento
...
### 14. AcpDetector — regex filtra paths absolutos (Hermes nunca detectado)

**Sintoma:** Hermes Agent nunca aparece no picker do AionUI, sem erro visível.

**Causa:** O `AcpDetector.ts` usa regex `/^[a-zA-Z0-9_.-]+$/` para filtrar comandos antes de os enviar para `command -v`. Esta regex rejeita qualquer string contendo `/` — logo, paths absolutos como `/Users/alvarobiano/.hermes/venv/bin/hermes` são silenciosamente descartados.

**Fix em `src/process/agent/acp/AcpDetector.ts` (linha ~56):**
```typescript
// ANTES — rejeita paths com /
cmd => /^[a-zA-Z0-9_.-]+$/

// DEPOIS — permite / em paths
cmd => /^[a-zA-Z0-9_./-]+$/
```

**Nota:** O `cliCommand` em `acpTypes.ts` configura apenas o nome do comando (`hermes`), não o path absoluto. O AcpDetector faz `command -v` com base no PATH do sistema — por isso o fix à regex é suficiente para permitir paths absolutos descobertos via PATH.

**Sessão:** 03/05/2026 — Hermes Agent露出了 no picker após patch.

### 15. better-sqlite3 rebuild para Electron (Node v136)

**Sintoma:** App não inicia após install — `native module not compiled for this Electron version`.

**Causa:** `better-sqlite3` é um módulo nativo. O build de npm não sabe que vai correr dentro do Electron (Node v136), não do Node do sistema.
...
**Quando aplicar:** Sempre que o app tem módulos nativos e o build é para packaged Electron (DMG/appImage). O target version deve coincidir com a versão do Electron usada no `package.json`.

**Sessão:** 03/05/2026 — rebuild necessário para Electron 37.10.3 no AionUI DMG.

### 16. gh CLI broken com Node 24 — alternativas

**Sintoma:** `gh workflow run` ou qualquer comando `gh` dá:
```
TypeError: Cannot read properties of undefined (reading 'options')
    at getAvailableArgsOnCmd
```

**Causa:** `gh` CLI no sistema usa Node 24 internamente, que tem breaking changes.

**Alternativas (por ordem de preferência):**
1. **API REST via Python urllib** — sempre funciona, não depende de gh CLI
2. **NVM** — `nvm use 22 && gh workflow run ...`
3. **UI web** — accionar manualmente em https://github.com/AlvaroBiano/AionUi/actions

```python
# Exemplo: dispatch workflow via API REST
import urllib.request, json, re
with open(os.path.expanduser('~/.netrc')) as f:
    token = re.search(r'password\s+(\S+)', f.read()).group(1)
...
**Sessão:** 03/05/2026 — gh broken, dispatch via Python API REST.

### 17. Bundle — lancedb é o package mais pesado do venv
...
### 23. Hermes blindagem permanente — auto-restore do patch após actualizações (03/05/2026)

**O problema:** O patch ao `app.asar` é perdido quando o AionUI é actualizado via novo DMG/instalador.

**Arquitectura da solução:**

```
~/Library/Application Support/hermes/aionui-hermes-patch/
├── app_backup.asar              ← app.asar JÁ PATCHED (backup permanente)
├── renderer/assets/
│   └── index-CVd-wlNn.js       ← renderer bundle PATCHED (copia permanente)
├── main/teamTypes.ts           ← source TypeScript PATCHED (para rebuilds futuros)
└── restore_hermes_patch.sh     ← script de reparação automática
```

**LaunchAgent** (`~/Library/LaunchAgents/com.bianinho.aionui-hermes-patch.plist`):
- Corre a cada **5 minutos** (StartInterval: 300)
- Também corre ao **fazer login** (RunAtLoad: true)
- Detecta se o `app.asar` perdeu o patch e re-aplica automaticamente

**Como funciona o restore script:**

```bash
# 1. Verifica se aAt já tem "hermes" → se tem, não faz nada
grep -q 'aAt=new Set(\["gemini","claude","codex","aionrs","hermes"\])' bundle.js

# 2. Se não tem, extrai o app.asar actual
npx asar extract "$ASAR_PATH" /tmp/restore$$

# 3. Faz sed do renderer bundle
sed -i 's/aAt=new Set(\["gemini","claude","codex","aionrs"\])/aAt=new Set(["gemini","claude","codex","aionrs","hermes"])/g' \
  /tmp/restore$$/out/renderer/assets/index-*.js

# 4. Repack + re-assina
npx asar pack /tmp/restore$$ "$ASAR_PATH"
codesign -f -s - /Applications/AionUi.app
```

**Verificação do LaunchAgent:**
```bash
launchctl print gui/$UID/com.bianinho.aionui-hermes-patch
# state = not running → está bem (só corre no intervalo)
# active count = 0 → não está a correr agora
```

**Se o patch precisar de ser forçado:**
```bash
# Correr o script manualmente
bash ~/Library/Application\ Support/hermes/aionui-hermes-patch/restore_hermes_patch.sh

# Ou forçar o LaunchAgent
launchctl start com.bianinho.aionui-hermes-patch
```
...
- `references/aionui-hermes-blindagem.md` — **Patch blindado do Hermes: dual-location fix (main+renderer) + launchd auto-restore + restore script completo (03/05/2026)**
- `references/aionhub-ecosystem.md` — **AionHub ecosystem: extensions, agents marketplace, CLI tools gratuitas (Goose + OpenClaw) (03/05/2026)**
- `references/aionui-dmg-patch-03052026.md` — **DMG patching: rebuild breaks native modules + signing, extract/pack works**
- `references/teams-mcp-architecture.md` — **Teams MCP: arquitectura, troubleshooting EACCES permission, debugging**
- `references/aionui-teams-hermes-limitation.md` — **Teams: Hermes não aparece como Team Leader — root cause, workarounds**
- `references/agent-selector-debug-03052026.md` — **Agent selector: Hermes desapareceu — HTTP bridge vs TCP, config decoding, Teams MCP**
...
- `references/aionui-interface-02052026.md` — **Interface real do AionUI: características, comparison DMG vs servidor**
- `references/aionui-fork-workflow.md` — fork setup, build, add page/IPC bridge
- `references/aionui-remote-connection.md` — **como ligar AionUI Mac ao Bianinho no servidor (Tailscale vs cloudflared vs bundle)**
- `references/dmg-build.md` — **DMG build: dmg-license macOS-only, publish:null fix, extraResources extraction**
- `references/asar-bundle-debug-case.md` — **Case study: 4 builds para resolver white screen | extracção ASAR + Python + SWR defensive pattern**
- `references/white-screen-debug-02052026.md`
- `references/cron-job-failures.md` — **Cron jobs falham com "Conversation not found" — fix: criar conversation + agent_config (05/05/2026)**
- `references/opcao-a-mac-bundle.md`
